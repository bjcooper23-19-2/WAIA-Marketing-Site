...
**Tally setting still required:** the published form currently returns successful submissions to `https://waia.co.uk/`. Change its post-submission redirect in Tally to exactly `https://waia.co.uk/workplace-ai-visibility-check/check/`. This setting cannot be changed by the static site. Do not append first name, email, organisation or other response values to the redirect URL. The destination route will become available on the public site when this branch is deployed.
