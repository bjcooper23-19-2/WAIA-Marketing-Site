...
The browser never contacts or executes code from Apps Script and has no Apps Script URL or CORS dependency. The Worker follows Google's Content Service redirects server-side, forwards the token with a separate private proxy secret, and normalises the response to `{ "ok": true }` or `{ "ok": false }` without record data. Neither opening the email nor loading the WAIA page calls the backend. Apps Script remains the token authority and preserves the Sheet lookup, expiry check, script lock, redemption timestamp and single-use behaviour.
