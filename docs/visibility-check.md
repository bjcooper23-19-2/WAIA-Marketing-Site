...
The `/start/` page is designed to link to a dedicated WAIA Tally form. **The form must be created and its URL must replace `TALLY_GATE_URL` before this branch is merged.** It should ask only for first name, work email and organisation, then redirect after a successful submission to `https://waia.co.uk/workplace-ai-visibility-check/check/`. The form itself should identify the lead source as **Workplace AI Visibility Check**. Tally will store contact details in the existing WAIA Tally workspace; they are not placed in site URLs, browser storage or WAIA static files. A WAIA workspace owner can open the form’s **Submissions** tab or export its responses for follow-up. Tally's duplicate prevention should use work email. A returning visitor can use the direct continuation link on `/start/` rather than create another submission.

The gate should not subscribe a visitor to marketing emails. There is no automatic nurture sequence. Any later promotional email must follow the current privacy notice and applicable consent or opt-out requirements. The link to the shared privacy notice is on the start page and must also be placed on the Tally form.
