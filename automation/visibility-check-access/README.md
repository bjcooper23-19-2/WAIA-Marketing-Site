...
4. Choose **Deploy → New deployment → Web app**. Set **Execute as: Me** and **Who has access: Anyone** so Tally and recipients can reach it without Google login. Approve the Sheets and Gmail scopes. Copy the production URL ending `/exec`, then add it as the `WEB_APP_URL` Script Property, with no query string. Because the property is read at runtime, this does not require a new deployment. If code changes later, deploy a new version through **Manage deployments → Edit**.

5. In the published Tally form `https://tally.so/r/GxPbRz`, **turn off Redirect on completion** to `https://waia.co.uk/workplace-ai-visibility-check/check/`. Set the thank-you message to:
...
6. Keep exactly the existing **First name**, **Work email**, and **Organisation** fields in that Tally form. Their labels must match those strings, ignoring case. Under **Integrations → Webhooks**, set the endpoint to `<WEB_APP_URL>?webhook_key=<WEBHOOK_SECRET>`. Do not share this URL. Tally's signing-secret option cannot be verified by Apps Script because Apps Script does not expose request headers. The private URL secret is the required check.

7. Submit the live Tally form once with an inbox you control. Confirm one new `Requests` row, `Token status = unused`, a seven-day `Expires at`, and one addressed Gmail draft containing the unique link. Check Tally's webhook event log for `Recorded; draft created`. If it says `Invalid payload`, compare its request body's `data.formId` and field labels with step 3 and step 6, correct the Script Property or labels, then submit again. Review and **manually send** the draft, then enter the send date/time in `Sent at`. Open the link: the confirmation page should appear and the Sheet must still say `unused`. Press **Continue to the Visibility Check**: the check should open and the Sheet should say `redeemed` with `Redeemed at`. Open the link again and confirm it says the link is no longer active. Delete the test row and draft only if appropriate to your records policy.
...
- Apps Script's HtmlService may restrict automatic top-level navigation after the asynchronous redemption call. The confirmation page offers a direct **Open the Visibility Check** link if the browser does not navigate automatically. The token is still marked redeemed by the first button press.
